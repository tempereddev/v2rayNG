...
object AngConfigManager {
     ...
     */
    fun getShareUrl(guid: String): String {
        return shareConfig(guid)
    }
        ...
        try {
            ...
            servers.lines()
                ...
                .forEach {
                    ...
                    if (config != null && configs.none { existing -> existing == config }) {
                        configs.add(config)
                    }
                }
            ...
            if (configs.isNotEmpty()) {
                val keyToProfile = if (append) {
                    ...
                    replaceSubscriptionConfigs(configs, subid)
                }
    ...
    /**
     * Replaces the entire config list of a subscription with a fresh set.
     * This avoids accidental accumulation when a subscription is refreshed repeatedly.
     */
    private fun replaceSubscriptionConfigs(configs: List<ProfileItem>, subid: String): Map<String, ProfileItem> {
        val keyToProfile = mutableMapOf<String, ProfileItem>()
    ...
    /**
     * Extracts valid subscription URLs from free-form text while ignoring ordinary config links.
     */
    ...
    fun updateConfigViaSingleSub(guid: String): SubscriptionUpdateResult {
        val subItem = MmkvManager.decodeSubscription(guid) ?: return SubscriptionUpdateResult()
